            ...
            <div class="Polaris-Card">
                <div class="Polaris-Card__Header Polaris-Card__Header-meta">
                    <div class="Polaris-Stack Polaris-Stack--alignmentBaseline">
                        ...
                        <import-export ref="importexport" :resourceType="resourceType" :lbl1="`Product Configurations`" :lbl2="`Variant Configurations`" :r2="`variants`" :shop="shop" :checkedIDs="checkedIDs" :is_data="productsList.length > 0"></import-export>
                    </div>
                </div>
                ...
                <div class="Polaris-Card__Section">
                    <div class="d-flex justify-content-between{">
                        ...
                        <button type="button" class="Polaris-Button Polaris-Button--primary ml-16" v-on:click="getProducts(1)"><span
                    ...
                    <div class="metafield-list">
                        ...
                        <thead>
                            <tr v-if="productsList.length > 0">
                                <th class="text-left" style="width: 18px;" id="multi-del" colspan="0">
                                    <multi-delete ref="multidelete" :resourceType="resourceType" :list="productsList" :next_page="next_page" :checkedIDs="checkedIDs" @update="updateChecked"></multi-delete>
                                </th>
                                <th class="text-left" v-if="checkedIDs.length <= 0">
                                    <h3><span class="Polaris-TextStyle--variationStrong">Products</span></h3>
                                </th>
                                <th class="text-left" v-if="checkedIDs.length <= 0">
                                ...
                                <th class="text-left" v-if="checkedIDs.length <= 0">
                                    <h3><span class="Polaris-TextStyle--variationStrong">Variant</span></h3>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                        <bullet-list-loader :width="300" :height="200" primaryColor="#f3f3f3" v-if="is_load">
                            ...
                            <tr class="Polaris-ResourceItem" v-else-if="productsList.length > 0" v-for="(product, index) in productsList" :key="index">
                                <td style="width: 18px;">
                                    <label class="Polaris-Choice" :for="`chkbox`+ index">
                                        <span class="Polaris-Choice__Control">
                                            <span class="Polaris-Checkbox">
                                                <input type="checkbox" class="Polaris-Checkbox__Input" aria-invalid="false" role="checkbox" aria-checked="false" value="" :id="`chkbox`+ index" :checked="product.is_checked" @click="checkSingle(product.id,index)">
              ...
              <path d="M8.315 13.859l-3.182-3.417a.506.506 0 0 1 0-.684l.643-.683a.437.437 0 0 1 .642 0l2.22 2.393 4.942-5.327a.437.437 0 0 1 .643 0l.643.684a.504.504 0 0 1 0 .683l-5.91 6.35a.437.437 0 0 1-.642 0"></path>
                                ...
                                <router-link
                                    :to="{name :'metafieldvalue', params: { resourceType : resourceType, resource: product, shop: shop  }}"
                                    tag="td" class="">
                                        <div class="d-flex align-center">
                                            <span class="mr-10 Polaris-Avatar Polaris-Avatar--styleSix Polaris-Avatar--sizeMedium Polaris-Avatar--hasImage border-radius-0">
                                                <img :src=product.image alt="" role="presentation" class="Polaris-Avatar__Image border-radius-0"></span>
                                ...
                                <router-link :to="{name :'metafieldvalue', params: { resourceType : resourceType, resource: product, shop:shop  }}" tag="td" class="">
                                    <div v-if="product.published_at === 'Hidden'" style="--top-bar-background:#00848e; --top-bar-background-lighter:#1d9ba4; --top-bar-color:#f9fafb;"><span class="Polaris-Badge Polaris-Badge--progressComplete"><span class="Polaris-Badge__Content">{{product.published_at}}</span></span></div>

                                    <div v-if="product.published_at === 'Visible'" style="--top-bar-background:#00848e; --top-bar-background-lighter:#1d9ba4; --top-bar-color:#f9fafb;"><span class="Polaris-Badge Polaris-Badge--statusSuccess"><span class="Polaris-Badge__Content">{{product.published_at}}</span></span></div>
                                </router-link>
                                <td>
                                    <router-link :to="{name :'variants', params: { resourceType : 'variants', base_id: product.id }}" tag="a" class="Polaris-Link">Variants
                                    </router-link>
                                </td>
                            </tr>
                            <tr v-else>
                                <td colspan="4" class="text-center">
                                    ...
                                    <h2 class="Polaris-Heading">Could not find any products</h2>
                    ...
                    <custom-paginate v-if="productsList.length > 0" :prev_page="prev_page" :next_page="next_page" :resourceType="resourceType"></custom-paginate>
    ...
    export default {
        ...
        methods: {
            async getProducts(page) {
                ...
                base.productsList = [];
                base.is_load = true;
                let endPoint = (page === 1) ? '/get-products?s=' + base.search : '/get-products?page=' + page + '&s=' + base.search;

                await axios.get(endPoint)
                    .then(res => {
                        var data = res.data.data;
                        base.productsList = data.product;
            ...
            checkSingle(id, index){
                ...
                base.isMultiCheck = false;
                base.productsList[index].is_checked = !base.productsList[index].is_checked;
            ...
            updateChecked(type){
                ...
                if( base.isMultiCheck ){
                    base.productsList.forEach(function(element){
                        element.is_checked = type;
                    });
                }else{
                    base.productsList.forEach(function(element , index){
